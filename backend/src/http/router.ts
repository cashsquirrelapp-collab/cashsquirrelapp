...
import recoverAccount from '../handlers/recover-account.js';
import confirmEmail from '../handlers/confirm-email.js';
const routes:Record<string,Handler>={auth,data,groups,profile,account,'recover-account':recoverAccount,'close-expired-accounts':closeExpiredAccounts,'admin-users':adminUsers,'confirm-email':confirmEmail,'download-report':download,'liff-config':liffConfig,'liff-submit':liffSubmit,'line-link-code':lineLink,'line-webhook':lineWebhook,notify,'password-reset-email':reset,'password-reset-line':reset,'send-monthly-report':report,'send-overdue-digest':digest,'setup-line-richmenu':setup,'stripe-webhook':stripe};
...
export function routeHandler(route:string):Handler {
 ...
 },{csrf:['auth','data','groups','profile','account','recover-account','admin-users','line-link-code','password-reset-email','password-reset-line','notify'].includes(route)});
}
