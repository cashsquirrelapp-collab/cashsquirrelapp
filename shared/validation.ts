...
const invoice=z.object({id,documentType:z.enum(['invoice','receipt','quotation']),documentNo:text,createdDate:text,issuer:profile,client:z.object({name:text,address:text,phone:text,email:text,taxId:text}),items:z.array(z.object({id,description:text,quantity:money,price:money})).max(1000),vatRate:z.number().min(0).max(100),whtRate:z.number().min(0).max(100)}).passthrough();
