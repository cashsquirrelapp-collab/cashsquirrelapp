...
const profile=z.object({name:text,address:text,phone:text,email:text,taxId:text,bankName:text,bankAccount:text,bankAccountName:text,logoUrl:z.string().max(500000).optional(),signatureUrl:z.string().max(500000).optional(),logoHeight:z.number().min(20).max(300).optional(),logoPosition:z.enum(['left','center','right','custom']).optional(),logoOffset:z.number().min(0).max(100).optional(),headerImageUrl:z.string().max(500000).optional(),headerImageHeight:z.number().min(20).max(400).optional(),headerImageOffset:z.number().min(0).max(100).optional(),website:text.optional()});
